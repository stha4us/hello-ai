...
class Sarima_TimeSeries(object):
    ...
    @delayed
    def _get_aic(self, exog, order, seasonal_order):
        try:
            model = SARIMAX(endog=exog, order=order, seasonal_order=seasonal_order).fit(disp=-1)
            return model.aic
        except:
            pass

    def _hyperparameter_optimize(self, parameters_list, d, s, q, exog):
        """
        ...
        for param in parameters_list:
            aic = self._get_aic(exog, (param[0], d, q), (param[1], D, param[2], s))
    ...
    def _model(self, lags, confidence, p, s):
        ...
        if self.optimize:
            ...
            order, season = self._hyperparameter_optimize(test_params, d, s, q, data)
        else:
            # Finding p , d and q
        ...
        # final model
        model = SARIMAX(data, exog=self.exog, order=order, seasonal_order=season,
